...
@triton.autotune(
    configs=[
        # Conservative configs with lower resource usage
        # Prioritize num_warps=2 to reduce SM resource contention and leave room for AllReduce
        triton.Config({}, num_warps=2, num_stages=3),
        triton.Config({}, num_warps=2, num_stages=2),
        triton.Config({}, num_warps=4, num_stages=2),
        # Removed num_warps=8 configs (too high resource usage causes AllReduce blocking)
    ],
    key=["BK", "BV", "K", "V"],
)
...
def fused_sigmoid_gating_delta_rule_update(
    ...
    # Use moderate block sizes to balance performance and resource usage
    BK = triton.next_power_of_2(K)
    BV = min(triton.next_power_of_2(V), 32)  # Conservative: use 32 to reduce SM resource contention
